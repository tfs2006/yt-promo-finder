...
export default async function handler(req, res) {
  ...
  try {
    const rawChannel1 = (req.query.channel1 || "").toString();
    const rawChannel2 = (req.query.channel2 || "").toString();
    
    const validation1 = validateChannelInput(rawChannel1);
    const validation2 = validateChannelInput(rawChannel2);
    
    if (!validation1.valid) {
      return res.status(400).json({ error: `Channel 1: ${validation1.error}` });
    }
    if (!validation2.valid) {
      return res.status(400).json({ error: `Channel 2: ${validation2.error}` });
    }
    
    const channel1 = validation1.sanitized;
    const channel2 = validation2.sanitized;
