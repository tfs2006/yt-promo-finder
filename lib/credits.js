...
function getSupabaseConfig() {
  return {
    supabaseUrl: asString(process.env.SUPABASE_URL || process.env.VITE_SUPABASE_URL, 400),
    serviceRoleKey: asString(process.env.SUPABASE_SERVICE_ROLE_KEY, 4000),
    anonKey: asString(process.env.SUPABASE_ANON_KEY || process.env.VITE_SUPABASE_ANON_KEY, 4000)
  };
}
...
function createSupabaseUserClient(accountToken) {
  const { supabaseUrl, anonKey } = getSupabaseConfig();
  if (!supabaseUrl || !anonKey || !looksLikeJwt(accountToken)) {
    return null;
  }
...
async function getLinkedAccountFromToken(accountToken) {
  ...
  const supabase = getSupabaseAdminClient() || createSupabaseUserClient(accountToken);
...
async function consumeAccountCredits(account, requiredCredits, accountToken = "") {
  const supabase = getSupabaseAdminClient() || createSupabaseUserClient(accountToken);
...
export async function getToolAccessState(req, toolKey) {
  ...
  if (accountState.eligible) {
    return {
      ...
      toolKey,
      requiredCredits,
      accountToken: accountState.accountToken,
...
export async function finalizeToolAccess(req, res, state, options = {}) {
  ...
  if (state.mode === "account") {
    ...
    const result = await consumeAccountCredits(state.account, state.requiredCredits, state.accountToken);
    if (!result.ok) {
      return {
        ok: false,
