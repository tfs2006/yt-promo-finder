...
const PIPED_FALLBACK_API = String(process.env.PIPED_FALLBACK_API || "https://api.piped.private.coffee")
  .trim()
  .replace(/\/+$/, "");
...
async function getInfoWithFallback(rawUrl) {
  ...
  try {
    const pipedInfo = await fetchPipedFallbackInfo(watchUrl);
    if (Array.isArray(pipedInfo?.formats) && pipedInfo.formats.length > 0) {
      return pipedInfo;
    }
  } catch (err) {
    lastError = err;
...
function toContainerFromMime(mimeType, fallback = "") {
  const value = String(mimeType || fallback).toLowerCase();
  if (value.includes("webm")) return "webm";
  if (value.includes("mp4") || value.includes("m4a")) return "mp4";
  return "mp4";
}

async function fetchPipedFallbackInfo(sourceUrl) {
  const normalized = normalizeYouTubeWatchUrl(sourceUrl);
  const videoId = YtdlCore.getURLVideoID(normalized);
  const endpoint = `${PIPED_FALLBACK_API}/streams/${encodeURIComponent(videoId)}`;
  ...
  if (!response.ok) {
    throw new Error("Piped fallback did not return stream data.");
  }
  ...
  const formats = [
    ...videoStreams
      ...
      .map((stream) => ({
        ...
        quality: { label: stream.quality || "" },
        mimeType: stream.mimeType || "video/mp4",
        container: toContainerFromMime(stream.mimeType, stream.format),
    ...
    ...audioStreams
      ...
      .map((stream) => ({
        ...
        container: toContainerFromMime(stream.mimeType, stream.format),
        bitrate: Number(stream.bitrate || 0),
        audioBitrate: parseKbps(stream.quality)
      }))
  ];

  if (!formats.length) {
    throw new Error("Piped fallback did not expose downloadable streams.");
  }

  return {
    videoDetails: {
      title: data?.title || "YouTube video",
      author: { name: data?.uploader || "YouTube creator" },
      ownerChannelName: data?.uploader || "YouTube creator",
      videoId,
      lengthSeconds: Number(data?.duration || 0),
      thumbnails: data?.thumbnailUrl ? [{ url: data.thumbnailUrl }] : []
    },
    formats,
    extractionMode: "piped"
  };
}
...
function getMetaFromInfo(info, sourceUrl) {
  ...
  const extractionMode = String(info?.extractionMode || "direct");
  const isFallbackMode = extractionMode !== "direct";
  ...
  return {
    ...
    extractionMode,
    warning: isFallbackMode
      ? "Using a fallback extraction route. If download fails, try again in a minute."
      : undefined
  };
}
