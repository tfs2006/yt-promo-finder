...
const PROVIDER_PAYLOAD_NUMERIC_FIELDS = new Set(["quantity", "runs", "interval", "min", "max", "posts", "delay", "expiry"]);
...
function buildProviderPayloadMetadata(providerPayload) {
  const metadata = {};
  for (const field of PROVIDER_PAYLOAD_FIELDS) {
    const value = toStripeMetadataValue(providerPayload?.[field], 500);
    if (!value) continue;
    metadata[`p_${field}`] = value;
  }
  return metadata;
}

function buildProviderPayloadFromSessionMetadata(session, service) {
  ...
  for (const field of PROVIDER_PAYLOAD_FIELDS) {
    let raw = asString(metadata?.[`p_${field}`], 2000);

    if (!raw && field === "link") {
      raw = asString(metadata?.link, 2000);
    }
    if (!raw && field === "quantity") {
      raw = asString(metadata?.quantity, 40);
    }

    if (!raw) continue;

    if (PROVIDER_PAYLOAD_NUMERIC_FIELDS.has(field)) {
      const numeric = asInteger(raw);
      if (numeric !== null) {
        payload[field] = String(numeric);
      }
      continue;
    }
  ...
  if (payload.link && !validatePublicLink(payload.link)) {
    throw new Error("Invalid link format. Use a full https:// URL.");
  }
...
async function updateStripeSessionMetadata(sessionId, metadataPatch) {
  const safeSessionId = asString(sessionId, 120);
  if (!safeSessionId || !metadataPatch || typeof metadataPatch !== "object") {
    return;
  }
  ...
  for (const [key, value] of Object.entries(metadataPatch)) {
    ...
    if (!safeKey) continue;
    cleanedPatch[safeKey] = toStripeMetadataValue(value, 500);
  }
  ...
  try {
    ...
    await stripe.checkout.sessions.update(safeSessionId, { metadata: cleanedPatch });
  } catch (error) {
    console.warn("Failed to update Stripe session metadata:", error?.message || error);
  }
}

async function recoverOrderFromStripeSession(sessionId, existingSession = null) {
  const safeSessionId = asString(sessionId, 120);
  ...
  if (existingSession && asString(existingSession?.id, 120) === safeSessionId) {
    session = existingSession;
  } else {
    try {
      session = await stripe.checkout.sessions.retrieve(safeSessionId);
    } catch (error) {
      if (asString(error?.code, 80) === "resource_missing") {
        return null;
      }
      throw error;
    }
  ...
  const service = services.find((item) => item.serviceId === serviceId) || null;

  const quantity = asInteger(session?.metadata?.quantity) || asInteger(session?.metadata?.p_quantity);
  const rate = Number.isFinite(service?.rate) ? service.rate : 0;
  ...
  const paymentStatus = asString(session?.payment_status, 40).toLowerCase();
  const checkoutStatus = asString(session?.status, 40).toLowerCase();
  const providerOrderId = asString(session?.metadata?.provider_order_id, 120) || null;
  const providerStatus = asString(session?.metadata?.provider_status, 120) || null;
  const providerError = asString(session?.metadata?.provider_error, 500) || null;

  let status = asString(session?.metadata?.order_status, 80).toLowerCase();
  if (!status) {
    if (providerOrderId) {
      status = statusFromProvider(providerStatus, "submitted");
    } else if (["paid", "no_payment_required"].includes(paymentStatus) || checkoutStatus === "complete") {
      status = "paid";
    } else {
  ...
  if (service) {
    try {
      providerPayload = buildProviderPayloadFromSessionMetadata(session, service);
    } catch (error) {
  ...
  return {
    id: `ord_recovered_${safeSessionId}`,
    createdAt,
    updatedAt: toIsoNow(),
    status,
    ...
    serviceType: asString(session?.metadata?.service_type, 120) || asString(service?.type, 120) || "",
    category: asString(service?.category, 120) || "",
    quantity: quantity || null,
    link: asString(session?.metadata?.link, 1000) || asString(session?.metadata?.p_link, 1000) || null,
    retailPrice,
    providerListCost: roundMoney(providerListCost),
    providerCost,
...
async function submitProviderOrder(order, options = {}) {
  if (!order || order.providerOrderId || !order.providerPayload) {
    return order;
  }

  const stripeSessionId = asString(order?.stripeSessionId, 120);
  if (!stripeSessionId) {
    return order;
  }

  const currentSessionMetadata = options.session?.metadata || {};
  const metadataProviderOrderId = asString(currentSessionMetadata?.provider_order_id, 120);

  if (metadataProviderOrderId) {
    order.providerOrderId = metadataProviderOrderId;
    order.providerStatus = asString(currentSessionMetadata?.provider_status, 100) || order.providerStatus || "Pending";
    order.status = asString(currentSessionMetadata?.order_status, 80) || statusFromProvider(order.providerStatus, "submitted");
    order.providerError = asString(currentSessionMetadata?.provider_error, 500) || null;
    return order;
  }
  ...
  try {
    ...
    order.providerStatus = asString(providerResponse?.status, 100) || "Pending";
    ...
    await updateStripeSessionMetadata(stripeSessionId, {
    ...
    await updateStripeSessionMetadata(stripeSessionId, {
      order_status: order.status,
      provider_error: order.providerError
    });
...
async function handleCheckoutCompleted(session, eventId) {
  ...
  if (!order) {
    order = await recoverOrderFromStripeSession(stripeSessionId, session);
  }
  ...
  await submitProviderOrder(order, { session });
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          ...
          price_data: {
            ...
            product_data: {
              name: service.name,
              description: `${service.category} G�� Service ${service.serviceId}`
            }
          }
        }
      ],
      metadata: {
        ...
        finance_version: "v1",
        ...buildProviderPayloadMetadata(providerPayload)
      }
    });
...
export async function handleSmmOrderStatus(req, res) {
  ...
  try {
    ...
    if (sessionId) {
      ...
      if (!order) {
        order = await recoverOrderFromStripeSession(sessionId);
        if (order) {
          await saveOrder(order);
          await indexOrder(order);
        }
    ...
    if (shouldRefresh && !order.providerOrderId && isPaidLikeOrder(order) && order.providerPayload) {
      await submitProviderOrder(order);
      order.updatedAt = toIsoNow();
      await saveOrder(order);
      await indexOrder(order);
    }
    ...
    if (shouldRefresh && order.providerOrderId) {
      try {
        ...
        await saveOrder(order);
        await indexOrder(order);
        await updateStripeSessionMetadata(order.stripeSessionId, {
          provider_status: order.providerStatus || "",
          order_status: order.status || ""
        });
      } catch (error) {
...
export async function handleSmmOrderResubmit(req, res) {
  if (applyApiGuards(req, res, { rateKey: "smm-order-resubmit", maxRequests: 20, windowMs: 60_000 })) return;
  ...
  try {
    ...
    if (order.providerOrderId) {
      return res.status(409).json({ error: "Order already has a providerOrderId and cannot be resubmitted." });
    }

    if (!isPaidLikeOrder(order)) {
      return res.status(409).json({ error: "Payment is not completed yet for this order." });
    }

    if (!order.providerPayload) {
      return res.status(409).json({ error: "Provider payload is missing. This order cannot be resubmitted automatically." });
    }
    ...
    if (!order.providerOrderId) {
      return res.status(502).json({
        error: order.providerError || "Resubmission failed at provider.",
        order: sanitizeOrder(order)
      });
    }
    ...
    const recentOrders = includeRecent
      ? mergedOrders
          ...
          .map((order) => ({
            ...
            providerStatus: order.providerStatus || null,
            providerError: order.providerError || null,
            canResubmit: !order.providerOrderId && isPaidLikeOrder(order) && (
              asString(order.status, 80) === "paid_submission_failed" || Boolean(order.providerError)
            ),
